                    ...
                    {STROKE_TEXTS[props.type].map((content, idx) => {
                        ...
                        return (
                                <div className = {`stroke-text`} key = {idx}>
                                    <h1 className={`main-title txt-${props.type}-${idx}`}><span className='chinese-char'>{content.chinese}</span>{content.title}</h1>
                                    <p>{content.text}</p>
                                </div>
                        )
                    })}
    ...
    return (
      <>
        ...
        <main className="landing">
          ...
          <h1 className="main-title margin-bottom-title"><span className='chinese-char'>{content.chinese}</span>{content.title}</h1>
          <StrokeAnimation type={this.props.type} />
          <ReadMore direction="down" content={`${content.text.split('.')[0]}`} link = '#main-specifics-content'/>
